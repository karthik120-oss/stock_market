...
def parse_analysis_file_and_check_levels(file_path):
        ...
        try:
            ...
            for stock in results:
                ...
                # Initialize status and next levels
                status = ""
                next_levels = ""
                
                # Status and next levels based on recommendation type
                if stock['is_buy_rec']:
                    # Determine current status for BUY stocks
                    current_level_name = "Pivot"
                    if stock['r3'] and stock['current_price'] > stock['r3']:
                        distance = ((stock['current_price'] - stock['r3']) / stock['r3']) * 100
                        status = f"Above R3 (+{distance:.1f}%)"
                        next_levels = "🎯 Target achieved!"
                    elif stock['r2'] and stock['current_price'] > stock['r2']:
                        distance = ((stock['current_price'] - stock['r2']) / stock['r2']) * 100
                        status = f"Above R2 (+{distance:.1f}%)"
                        next_levels = f"R3: ₹{stock['r3']:.2f}" if stock['r3'] else "R3: N/A"
                    elif stock['r1'] and stock['current_price'] > stock['r1']:
                        distance = ((stock['current_price'] - stock['r1']) / stock['r1']) * 100
                        status = f"Above R1 (+{distance:.1f}%)"
                        levels = []
                        if stock['r2']: levels.append(f"R2: ₹{stock['r2']:.2f}")
                        if stock['r3']: levels.append(f"R3: ₹{stock['r3']:.2f}")
                        next_levels = " | ".join(levels) if levels else "R2/R3: N/A"
                    elif stock['current_price'] > stock['pivot']:
                        distance = ((stock['current_price'] - stock['pivot']) / stock['pivot']) * 100
                        status = f"Above Pivot (+{distance:.1f}%)"
                        levels = []
                        if stock['r1']: levels.append(f"R1: ₹{stock['r1']:.2f}")
                        if stock['r2']: levels.append(f"R2: ₹{stock['r2']:.2f}")
                        if stock['r3']: levels.append(f"R3: ₹{stock['r3']:.2f}")
                        next_levels = " | ".join(levels) if levels else "R1/R2/R3: N/A"
                    else:
                        distance = ((stock['pivot'] - stock['current_price']) / stock['pivot']) * 100
                        status = f"Below Pivot (-{distance:.1f}%)"
                        next_levels = f"Target: ₹{stock['pivot']:.2f} (Pivot)"
                        
                elif stock['is_sell_rec']:
                    # Determine current status for SELL stocks
                    if stock['s3'] and stock['current_price'] < stock['s3']:
                        distance = ((stock['s3'] - stock['current_price']) / stock['s3']) * 100
                        status = f"Below S3 (-{distance:.1f}%)"
                        next_levels = "🎯 Target achieved!"
                    elif stock['s2'] and stock['current_price'] < stock['s2']:
                        distance = ((stock['s2'] - stock['current_price']) / stock['s2']) * 100
                        status = f"Below S2 (-{distance:.1f}%)"
                        next_levels = f"S3: ₹{stock['s3']:.2f}" if stock['s3'] else "S3: N/A"
                    elif stock['s1'] and stock['current_price'] < stock['s1']:
                        distance = ((stock['s1'] - stock['current_price']) / stock['s1']) * 100
                        status = f"Below S1 (-{distance:.1f}%)"
                        levels = []
                        if stock['s2']: levels.append(f"S2: ₹{stock['s2']:.2f}")
                        if stock['s3']: levels.append(f"S3: ₹{stock['s3']:.2f}")
                        next_levels = " | ".join(levels) if levels else "S2/S3: N/A"
                    elif stock['current_price'] < stock['pivot']:
                        distance = ((stock['pivot'] - stock['current_price']) / stock['pivot']) * 100
                        status = f"Below Pivot (-{distance:.1f}%)"
                        levels = []
                        if stock['s1']: levels.append(f"S1: ₹{stock['s1']:.2f}")
                        if stock['s2']: levels.append(f"S2: ₹{stock['s2']:.2f}")
                        if stock['s3']: levels.append(f"S3: ₹{stock['s3']:.2f}")
                        next_levels = " | ".join(levels) if levels else "S1/S2/S3: N/A"
                    else:
                        distance = ((stock['current_price'] - stock['pivot']) / stock['pivot']) * 100
                        status = f"Above Pivot (+{distance:.1f}%)"
                        next_levels = f"Target: ₹{stock['pivot']:.2f} (Pivot)"
                    ...
                    # For HOLD/other recommendations
